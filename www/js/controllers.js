angular.module('app.controllers', [])
	...
	.controller('HomeCtrl', function ($scope, $rootScope, $state, $ionicSlideBoxDelegate) {
		...
		$scope.goFirst = function ($index) {
			var $slideCount = $ionicSlideBoxDelegate.$getByHandle('image-viewer').slidesCount();
		...
		//banner
		$rootScope.service.get('getBannerBlock', {block: 'main-slider'}, function (results) {
		...
		$rootScope.service.get('menus', {}, function (results) {
			var cats = [];
			for (var key in results) {
				var catChildren = results[key].children;//.split(',');
				for (var x in catChildren) {
					catChildren[x] = {childCatId: catChildren[x], show: false};
				}
				results[key].children = catChildren;
				cats.push({cat: results[key], show: false});
			}
			$scope.cats = cats;
			 ...
			 */
			$scope.toggleGroup = function (item, items) {
				for (var x in items) {
					items[x].show = false;
				}
				if (item.show) {
